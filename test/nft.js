...
describe('NFT', function () {
  ...
  it("increases token's id after each mint", async function () {
    await expect(nftContract.mintToken('')).to.emit(nftContract, 'TokenMinted').withArgs(1, '', marketplaceAddress);

    await expect(nftContract.mintToken('')).to.emit(nftContract, 'TokenMinted').withArgs(2, '', marketplaceAddress);
  });
