...
describe('Marketplace', function () {
  ...
  it('creates a Market Item', async function () {
    ...
    await expect(mintTokenAndCreateMarketItem(tokenId, price, transactionOptions))
      ...
      .withArgs(1, nftContractAddress, tokenId, owner.address, owner.address, ethers.constants.AddressZero, price, false, false);
  });
  ...
  it('reverts when trying to cancel an inexistent market item', async () => {
    // Act and Assert
    expect(marketplaceContract.cancelMarketItem(nftContractAddress, 1)).to.be.revertedWith('Market item has to exist');
  });
  ...
  it('reverts when trying to cancel a market item whose seller is not msg.sender', async () => {
    ...
    // Act and Assert
    expect(marketplaceContract.connect(buyer).cancelMarketItem(nftContractAddress, 1)).to.be.revertedWith('You are not the seller');
  });
  ...
  it('reverts a Market Item creation if listing fee is not right', async function () {
    ...
    // Act and Assert
    expect(mintTokenAndCreateMarketItem(tokenId, price, transactionOptions)).to.be.revertedWith('Price must be equal to listing price');
  });
  ...
  it('reverts a Market Item creation if price is 0', async function () {
    ...
    // Act and Assert
    expect(mintTokenAndCreateMarketItem(tokenId, price, transactionOptions)).to.be.revertedWith('Price must be at least 1 wei');
  });
  ...
  it('reverts a Market Sale if offer price is not the same as listing price', async function () {
    ...
    // Act and Assert
    expect(marketplaceContract.connect(buyer).createMarketSale(nftContractAddress, 1, { value: offerPrice })).to.be.revertedWith(
